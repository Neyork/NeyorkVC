...
async def update(_, message: Message):
    ...
    if str(m[0]) != "A":
        x = await message.reply_text("تم العثور على تحديثات! دفع الآن.")
        await start_restart_stage(x.chat.id, x.message_id)
        os.execvp("python3", ["python3", "-m", "Music"])
    else:
        await message.reply_text("بالفعل حتى الآن مع أحدث إصدار")
...
async def executor(client, message):
    if len(message.command) < 2:
        return await edit_or_reply(message, text="__Nigga أعطني بعض الأوامر للتنفيذ.__")
    try:
